...
V13의 `idx_ai_job_user_requested (user_id, requested_at)`는 기존 DB index이며, Part2 로컬 DBML의 누락된 index 표기만 동일하게 보완합니다. 새로운 migration이나 DB 제약은 추가하지 않습니다. 프로젝트 규칙에 따라 `docs/harness/08-current-status.md`와 `daily-log`를 유지하고, 작업 계획용 `docs/tasks` 문서는 PR에서 제외합니다.