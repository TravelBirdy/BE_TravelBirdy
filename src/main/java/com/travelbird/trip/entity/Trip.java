...
import com.travelbird.region.entity.SigunguMaster;
...
public class Trip {
  ...
  @ManyToOne(fetch=FetchType.LAZY,optional=false) @JoinColumn(name="sigungu_code",nullable=false) private SigunguMaster region;
  ...
  public static Trip createManual(User user,SigunguMaster region,LocalDate start,LocalDate end,CompanionType companion,Set<TravelTheme> themes,Pace pace,LocalDateTime now){
    ...
    var t=new Trip();t.user=user;t.region=region;t.startDate=start;t.endDate=end;t.companionType=companion;t.themes.addAll(themes);t.pace=pace;t.sourceType=TripSourceType.MANUAL;t.visibility=Visibility.PRIVATE;t.title="여행 일정";
  ...
  public boolean ownedBy(Long userId){return user!=null&&Objects.equals(user.getId(),userId);}
  ...
  public TripPlace addPlace(int dayNumber,Place place,Long placeId){return addPlace(dayNumber,place,placeId,null);} public TripPlace addPlace(int dayNumber,Place place,Long placeId,Integer requestedOrder){ensureMutable();if(days.stream().flatMap(d->d.getPlaces().stream()).anyMatch(p->Objects.equals(p.getPlace().getId(),placeId)))throw new BusinessException(ErrorCode.PLACE_ALREADY_ADDED);var d=day(dayNumber);if(d.getPlaces().size()>=15)throw new BusinessException(ErrorCode.TRIP_DAY_PLACE_LIMIT_EXCEEDED);int order=requestedOrder==null?d.getPlaces().size()+1:Math.max(1,Math.min(requestedOrder,d.getPlaces().size()+1));for(var p:d.getPlaces())if(p.getVisitOrder()>=order)p.changeOrder(p.getVisitOrder()+1);return d.addPlace(place,order);}
  public void addWishlistPlace(Long placeId){ensureMutable();if(days.stream().flatMap(d->d.getPlaces().stream()).anyMatch(p->p.getPlace()!=null&&Objects.equals(p.getPlace().getId(),placeId)))throw new BusinessException(ErrorCode.PLACE_ALREADY_ASSIGNED_TO_DAY);}
  public void applyAiPreview(List<com.travelbird.ai.entity.AiPreviewDay> route){ensureMutable();var existing=new HashMap<Long,TripPlace>();days.stream().flatMap(d->d.getPlaces().stream()).forEach(p->existing.put(p.getPlace().getId(),p));existing.values().forEach(p->p.changeOrder(p.getVisitOrder()+1000));for(var d:route)for(var pp:d.getPlaces()){var current=existing.remove(pp.getPlace().getId());if(current==null){var added=day(d.getDayNumber()).addPlace(pp.getPlace(),pp.getVisitOrder());added.setReason(pp.getReason());}else current.relocate(day(d.getDayNumber()),pp.getVisitOrder());}for(var obsolete:existing.values())obsolete.getDay().getPlaces().remove(obsolete);}
