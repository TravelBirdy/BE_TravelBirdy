...
개발·통합테스트·배포의 DB 기준은 MySQL 8이다. 통합테스트는 Docker/Testcontainers/H2 없이 각 개발자의 MySQL 8에 직접 연결한다. TEST_DB_HOST, TEST_DB_PORT, TEST_DB_USERNAME, TEST_DB_PASSWORD로 설정하며 계정·비밀번호는 공유하거나 커밋하지 않는다. 개발 DB travelbird를 사용하지 않고 실행 시 생성한 travelbird_test_* 전용 schema에서 Flyway 전체 migration과 JPA validate를 검증한 뒤 정리한다.
